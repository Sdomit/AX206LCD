...
> **Status 2026-06-28: Phase 0 COMPLETE.** AX206 panel driven over WinUSB — 480×320 RGB565, native orientation, 2.8 fps full-frame, reconnect OK. Evidence: [device-compatibility.md](device-compatibility.md), `device-profiles/ax206_usbdisplay.json`, `scripts/phase0/`. **Stack DECIDED (Path C, 2026-06-28):** Node/TypeScript engine + React Studio + .NET 9 ProbeHost child, Electron shell, `node-usb` driver — Go/SensorPanel fork dropped. ADR + scores: [architecture.md](architecture.md). **Progress through Slice 2A:** 1A — TS `node-usb` driver drives the panel (red CSW=0); 1B — device state machine + auto-reconnect + frame scheduler + headless service; 2A — .NET ProbeHost streams live CPU/RAM telemetry over JSON Lines, supervised + null-safe (CPU temp reports unavailable, never 0). **2B DONE** — live CPU-load + RAM bars render on the real panel from ProbeHost telemetry (threshold colors; null/stale shown explicitly). 19/19 unit tests. **2C DONE** — approved themed UI ported to the panel: CPU arc gauge, RAM card, status, custom 5×7 bitmap font; null/stale-safe; fixed a stale-halt open timeout and an unlistened-error crash. 22/22 unit tests. **2D DONE** — ProbeHost expanded to GPU/disk/network + CPU/GPU temps (schema v2); themed GPU/RAM/NET cards on the panel; robust open (drains a stale pipe + retries dimensions). GPU temp live; CPU temp needs an elevated run; disk temp unavailable on this drive (shown honestly). 18 unit tests. **5A DONE** — Claude 5h-window usage on the panel: reads local Claude Code logs (`~/.claude/projects/**/*.jsonl`, token counts only, no content), progress bar vs a user-set `CLAUDE_5H_TOKEN_LIMIT`; Codex honest-unavailable (`CODEX --`). 21 unit tests. **1C DONE** — Electron tray shell supervises the engine under system Node (no electron-rebuild needed): start/stop/restart engine, start-at-login toggle, quit; tray icon generated in-code. Phase 1 tray requirement met. **PHASE 3 DONE** — 3A: schema-driven renderer shared by engine + Studio (one `renderProfile` over an abstract Surface → preview == panel); 3C: **OrbitPanel Studio** (`apps/studio`, React+Vite) — canvas preview, widget palette (7 types), select + drag-move, inspector (geometry + props JSON), undo/redo, profile save/load/export/import, simulated telemetry. Verified rendering in-browser. Remaining Phase 3 niceties: snap/align guides, z-order UI, per-field property editors. **Quality pass DONE** — supersample 2× anti-aliasing + Bayer ordered dithering (kills RGB565 banding) + dirty-region partial blits (only the changed rectangle over USB; identical frames skip the transfer) + bigger hero font. Engine + Studio share the renderer. (Phase 4 dirty-region ✓.) Next: packaging (Phase 6), Phase 2 audio, or Studio polish (snap/align, z-order).
